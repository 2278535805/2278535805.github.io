		...
		if(cnt % 2) frameAnimater.stop();
			...
			if(app.playMode == 1){
				...
				$id('get-taxy').onclick = function(){
					...
					saveAs(new File([(inputName.value || inputName.placeholder) + '\n' + (inputLevel.value || inputLevel.placeholder) + (inputLevelLv.value || inputLevelLv.placeholder) + numlvl + '\n' + stat.taxy], 'TAXY', { type: "charset=utf-8" }));
				}
			}
			fucktemp1 = true;
			qwqPause();
...
function qwqdraw3(statData) {
	...
	if(ui.choice == '1'){
        ...
        ctxos.drawImage(ranks[status], ...conv(0.74, 0.25), ...conv(0.15, 0.25));
        //曲名+等级
		ctxos.fillStyle = '#fff';
		...
		if (dxsnm > conv(0.4, 0)[0]) ctxos.font = `${conv(0, 0.08)[1]/dxsnm*conv(0.3, 0)[0]}px ${fonts.choice},Noto Sans SC`;
		ctxos.textAlign = 'center';
		ctxos.fillText(inputName.value || inputName.placeholder, ...conv(0.35, 0.4));
		ctxos.font = `${conv(0, 0.08)[1]}px ${fonts.choice},Noto Sans SC`;
		ctxos.fillText((inputLevel.value || inputLevel.placeholder) + (inputLevelLv.value || inputLevelLv.placeholder) + (numLevel.value || numLevel.placeholder), ...conv(0.3, 0.65));
		//分数
		ctxos.fillStyle = '#fff';
		ctxos.textAlign = 'left';
		console.log(conv(0, 0.07)[1])
		ctxos.font = `${conv(0, 0.07)[1]}px ${fonts.choice},Noto Sans SC`;
		ctxos.fillText(stat.scoreStrNum, ...conv(0.58, 0.4));
		ctxos.font = `${conv(0, 0.03)[1]}px ${fonts.choice},Noto Sans SC`;
		if(app.playMode == 1){
			ctxos.fillStyle = '#ffc500';
			ctxos.fillText(typeStr, ...conv(0.58, 0.45));
		}else if(statData.scoreDelta > 0){
			ctxos.fillText('NEW BEST ' + toScoreStr(statData.scoreBest - statData.scoreDelta) + ' +' + toScoreStr(statData.scoreDelta), ...conv(0.58, 0.45));
		}
		//MaxCombo+Acc
		ctxos.fillStyle = '#fff';
		ctxos.font = `${conv(0, 0.04)[1]}px ${fonts.choice},Noto Sans SC`;
		ctxos.fillText(stat.maxcombo, ...conv(0.56, 0.57));
		ctxos.textAlign = 'right';
		ctxos.fillText(stat.accStr, ...conv(0.85, 0.57));
		/*if(hyperMode.checked){
			ctxos.textAlign = 'center';
		...
		ctxos.font = `${conv(0, 0.03)[1]}px ${fonts.choice},Noto Sans SC`;
		ctxos.textAlign = 'center';
		ctxos.fillText(stat.perfect, ...conv(0.55, 0.69));
		ctxos.fillText(stat.good, ...conv(0.60, 0.69));
		ctxos.fillText(stat.noteRank[6], ...conv(0.65, 0.69));
		ctxos.fillText(stat.noteRank[2], ...conv(0.69, 0.69));
		ctxos.font = `${conv(0, 0.02)[1]}px ${fonts.choice},Noto Sans SC`;
		ctxos.textAlign = 'right';
		ctxos.fillText(stat.noteRank[7], ...conv(0.82, 0.692));
		ctxos.fillText(stat.noteRank[3], ...conv(0.82, 0.717));
		//data
		ctxos.textAlign = 'left';
		ctxos.fillText(stat.data_kb, ...conv(0.54, 0.94));
		//rks
		ctxos.font = `${conv(0, 0.03)[1]}px ${fonts.choice},Noto Sans SC`;
		ctxos.fillStyle = '#000';
		ctxos.fillText(`单曲RKS:`, ...conv(0.85, 0.07));
		ctxos.fillText(stat.rks, ...conv(0.85, 0.1));
	}
}
...
function imgBlur(img) {
	...
	StackBlur.canvasRGBA(canvas, 0, 0, w, h, Math.ceil(Math.min(w, h) * 0.1000));
...
const inputLevelLv = $id('input-level-Lv');
...
async function qwqStop() {
	if (emitter.eq('stop')) {
		...
		stat.setConfig('change', change_text.checked);
		stat.setConfig('mode', mode.choice);
		levelText = (inputLevel.value || inputLevel.placeholder) + (inputLevelLv.value || inputLevelLv.placeholder) + (numLevel.value || numLevel.placeholder);
